...
export const getEmployee = (id: number) => {
  ...
  return api.get(`/user/employees/${id}`, {
    // Sesuaikan endpoint sesuai routes/api.php
    headers: {
...
export const createEmployee = (data: any) => {
  // Ubah type data jika Anda mengirim JSON
  ...
  return api.post("/admin/employees", data, {
    // Endpoint yang benar untuk admin membuat karyawan
    headers: {
...
export const updateEmployee = (id: string | number, data: FormData) => {
  ...
  return api.post(`/user/employees/${id}`, data, {
    // Sesuaikan endpoint sesuai routes/api.php
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
export const deleteEmployee = async (id: number) => {
  ...
  return await api.delete(`/user/employees/${id}`, {
    // Sesuaikan endpoint sesuai routes/api.php
    headers: {
      Authorization: `Bearer ${adminToken}`,
    },
...
export const removeAchievement = async (id: number) => {
  const token =
    localStorage.getItem("admin_auth_token") ||
    localStorage.getItem("employee_auth_token"); // Asumsi bisa admin atau karyawan (untuk achievement sendiri)
  if (!token) {
    throw new Error("Token otentikasi tidak ditemukan.");
  }
  return await api.delete(`/user/employees/achievements/${id}`, {
    // Sesuaikan endpoint sesuai routes/api.php
    headers: {
      Authorization: `Bearer ${token}`,
    },
